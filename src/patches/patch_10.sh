...
#     temp=$(mktemp) ; count=0
#     while IFS= read -r x ; do
#        if [[ $x =~ ^CookieAuthFileGroupReadable[[:space:]]1 ]] ; let count++ ; if [[ $count == 2 ]] ; then continue ; fi ; fi
#        echo "$x" >> $temp
#     done < <(sudo cat $torrc)
# done
# rm $temp
...
#     temp=$(mktemp) ; count=0
#     while IFS= read -r x ; do
#        if [[ $x =~ ^DataDirectoryGroupReadable[[:space:]]1 ]] ; let count++ ; if [[ $count == 2 ]] ; then continue ; fi ; fi
#        echo "$x" >> $temp
#     done < <(sudo cat $torrc)
# done
# rm $temp
