...
#     temp=$(mktemp) ; count=0
#     while IFS= read -r x ; do
#        if [[ $x =~ ^CookieAuthentication 1 ]] ; then let count++ ; if [[ $count == 2 ]] ; then continue ; fi ; fi
#        echo "$x" >> $temp
#     done < <(sudo cat $torrc)
# done
# rm $temp
...
#     temp=$(mktemp) ; count=0
#     while IFS= read -r x ; do
#        if [[ $x =~ ^CookieAuthFileGroupReadable 1 ]] ; let count++ ; if [[ $count == 2 ]] ; then continue ; fi ; fi
#        echo "$x" >> $temp
#     done < <(sudo cat $torrc)
# done
# rm $temp
...
#     temp=$(mktemp) ; count=0
#     while IFS= read -r x ; do
#        if [[ $x =~ ^DataDirectoryGroupReadable 1 ]] ; let count++ ; if [[ $count == 2 ]] ; then continue ; fi ; fi
#        echo "$x" >> $temp
#     done < <(sudo cat $torrc)
# done
# rm $temp
