...
podman ps >$dn 2>&1 || { announce "Docker not running. Aborting." && return 1 ; }
check_mempool_tmux || return 1
pn_tmux "
cd $hp/mempool/podman ; podman compose up -d ; cd - >$dn
" "starting_mempool"
...
check_mempool_tmux || return 1
pn_tmux "
cd $hp/mempool/podman ; podman compose stop ; cd - >$dn
" "stopping_mempool"
}
