...
export mempoolconf="$hp/mempool/podman/podman-compose.yml"
nogsedtest
...
if podman ps 2>$dn | grep -q mempool_web && \
   ...
   podman ps 2>$dn | grep maria | grep -q podman-db ; then
...
log1)
NODAEMON="true"
pn_tmux "podman logs podman-api-1 2>&1 | less" ; unset NODAEMON
unset NODAEMON
;;
log2)
NODAEMON="true"
pn_tmux "podman logs podman-db-1 2>&1 | less" ; unset NODAEMON 
unset NODAEMON
;;
log3)
NODAEMON="true"
pn_tmux "podman logs podman-mempool_web-1 2>&1 | less" ; unset NODAEMON 
unset NODAEMON
;;
...
announce "Choosee api$cyan (1)$orange, db$cyan (2)$orange, or web$cyan (3)$orange."
    ...
    case $enter_cont in
    1|api)
        pn_tmux "podman logs podman-api-1 2>&1 | less" ; unset NODAEMON ; break
        ;;
    2|db)
        pn_tmux "podman logs podman-db-1 2>&1 | less" ; unset NODAEMON ; break 
        ;;
    3|web)
        pn_tmux "podman logs podman-mempool_web-1 2>&1 | less" ; unset NODAEMON ; break
        ;;
    *)
        invalid 
        ;;
...
function list_mempool_podman_IPs {

rm $dp/podman_IPs >$dn 2>&1
podman inspect -f '{{.Name}}={{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' $(podman ps -q) \
| gsed 's/^\///' | while read theip ; do echo $theip | tee -a $dp/podman_IPs >$dn 2>&1 ; done

grep "podman-mempool_web-1" $dp/podman_IPs > $dp/mempool_IPs
grep "podman-db-1" $dp/podman_IPs >> $dp/mempool_IPs
grep "podman-api-1" $dp/podman_IPs >> $dp/mempool_IPs
}
