...
# podman run command ... -v $HOME/parmanode/fulcrum/config:/home/parman/parmanode/fulcrum/config \

[[ -z $coinfgure_bitcoin_self ]] && source $bc >$dn 2>&1

# set datadir variable

    if [[ $fulcrumpodman != "true" && $drive_fulcrum == "external" ]] ; then #works Linux and Mac
    datadir="$parmanode_drive/fulcrum_db" 
    fi

    if [[ $fulcrumpodman != "true" && $drive_fulcrum == "internal" ]] ; then
...
if [[ $fulcrumpodman == "true" && $OS == Mac ]] ; then local bitcoinIP="host.podman.internal" ; else local bitcoinIP="127.0.0.1" ; fi
