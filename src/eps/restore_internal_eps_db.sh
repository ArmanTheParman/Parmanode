function restore_internal_eps_db {
#offers to restore an internal drive backup if found


file="$hp/eps/eps_db" #this is the old internal location, necessary to leave for this function.
file2="$hp/eps_db_backup"
file3="$HOME/.eps_db_backup"
unset found message
...
########################################################################################
    ...
    $HOME/.eps_db$orange but that directory already exists.
$red
     a)    Abort, Abort ! 
$pink
     b)    Do it anyway, I don't care if I lose what's in $HOME/.eps_db
$green    
     c)    Abandon the move and just use whats in $HOME/.eps_db
$orange
########################################################################################    
" ; choose "xpmq" ; read choice 
jump $choice || { invalid ; continue ; } ; set_terminal
