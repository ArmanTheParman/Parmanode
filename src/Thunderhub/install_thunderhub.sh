...
# export envfile=$hp/thunderhub/.env 
export envfile="$hp/thunderhub/.env.local" #adding '.local' prevents overriding envfile when updating thunderhub.
sned_sats
##### export LNDIP
...
if ! grep -q "lnd-end" $ic >$dn 2>&1 && ! grep -q "lnddocker-end" $ic >$dn 2>&1 && ! [[ $debug == 1 ]] ; \
then
...
debug "check .env.local envfile exists:
$cyan
...
unset version envfile password password2
