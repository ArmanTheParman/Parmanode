...
########################################################################################
    ...
    computer. If that were to fail, then your seed back up is not good, and the 
    wallet you're about to delete is your only copy. If you do test it, do it offline,
    check the addresses generated are correct, and then delete it. It should be
    offline so that there is not conflicts with the existing LND node.
