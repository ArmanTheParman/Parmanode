...
if [[ -z $ipcore ]] ; then ipcore="127.0.0.1"
    ...
    if [[ -n $remote_user]] && [[ $remote_pass ]] ; then
        rpcuser=$remote_user
        rpcpass=$remote_pass
    else
...
echo "
; LND conf configuration, message added by Parmanode, from version 3.33.2
...
;
; the externalip and tlsextraip setting allows access using an IP address.
; the externalhosts and tlsextradomain setting allows access using a domain name.
