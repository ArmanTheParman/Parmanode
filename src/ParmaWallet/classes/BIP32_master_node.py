...
class BIP32_master_node:
    def __init__(self, mnemonic=None, byteseed=None): #Depth=0, Derivation path is m (not m/0), so "index" meaningless at this level.
        ...
        raw_xprv = xprv_prefix + self.depth + fp + child + self.chain_code + self.priv_key_33b
    ...
    def __repr__(self):
        return "Master Node BIP32 xprv is: {}".format(self.xprv) + "\n" \
        "Master Node BIP32 xpub is: {}".format(self.xpub)

class child_keys:
    def __init__(self, parent: Union[BIP32_master_node, 'child_keys'], account=0, hardened=True, ): #account is also the "index"
        ...
        while True:
            if hardened:
                I2 = hmac.new(parent.chain_code, parent.priv_key_33b + int.to_bytes(i , 4, 'big'), hashlib.sha512).digest()  # (key, data, hash alogrithm)
                Il2, self.chain_code = I2[:32], I2[32:]
                child_private_key = (int.from_bytes(Il2, 'big') + int.from_bytes(parent.priv_key_33b[1:], 'big')) % N  #arithmatic, not concatenation.
                self.private_key = PrivateKey(child_private_key)
                self.private_key_33b = b'\0' + self.private_key.secret_bytes
                self.public_key = self.private_key.point.sec()
            else: #not finished
                I2 = hmac.new(parent.chain_code, parent.pub_key + int.to_bytes(i , 4, 'big'), hashlib.sha512).digest()  # (key, data, hash alogrithm)
                Il2, self.chain_code = I2[:32], I2[32:]
                child_private_key = (int.from_bytes(Il2, 'big') + int.from_bytes(parent.priv_key_33b[1:], 'big')) % N  #arithmatic, not concatenation.
                self.private_key = PrivateKey(child_private_key)
                self.private_key_33b = b'\0' + self.private_key.secret_bytes
        ...
        fp_from_parent = hash160(parent.public_key)[:4]
