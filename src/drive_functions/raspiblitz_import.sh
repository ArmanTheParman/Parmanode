...
#Get disk ID
export disk=$(sudo blkid | grep BLOCKCHAIN | cut -d : -f 1) 
...
if [[ -d $mount_point/.bitcoin ]] ; then sudo mv $mount_point/.bitcoin $mount_point/.bitcoin_backup_0 
    ...
    sudo rm $mount_point/.bitcoin >$dn 2>&1 #deletes symbolic link
fi
...
########################################################################################
    ...
    Would you like to$cyan replace the old Parmanode drive$orane with the new drive from 
