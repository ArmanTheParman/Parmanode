...
debug "check disk mounted in container"
...
debug "check symlink created and permission set"
docker exec -it umbrel /bin/bash
...
debug "check parmanode_backedup exists, conf file moved, and permission to bitcoin dir set"
docker exec -it umbrel /bin/bash

make_bitcoin_conf "umbrel"  #(not umbrel-drive or $log, because overriding)

debug "check bitcoin conf made"
docker exec -it umbrel /bin/bash
...
debug "check drive label"

docker exec -it umbrel /bin/bash -c "umount $mount_point"

debug "check unmounted from container"
docker exec -it umbrel /bin/bash
