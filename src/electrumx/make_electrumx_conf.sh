...
LOG_LEVEL = info # options are debug, info, warning, and error 
ANON_LOGS = do_not_track,motherfucker
LOG_SESSIONS = 0
MAX_SESSIONS = 200 # reduced from default of 1000
REQUEST_TIMEOUT = 50 # seconds
BANNER_FILE = $hp/electrumx/banner.txt
TOR_BANNER_FILE = $hp/electrumx/torbanner.txt
# DONATION_ADDRESS = 
PEER_DISCOVERY = on
PEER_ANNOUNCE = on # To disable must set to empty, eg PEER_ANNOUNCE = 
DAEMON_URL = http://$rpcuser:$rpcpassword@127.0.0.1:8332/
COIN = Bitcoin  # don't shitcon, remember the Parmanode policy, there's only 1 rule!
