...
#&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&&
if [[ $reusedotlitd != "true" ]] ; then
create_wallet && litd_wallet_unlock_password  # && because 2nd command necessary to create

# gsed=sed alias normally works, but failing here.
if [[ $OS == Linux ]] ; then
sed -i '/^; wallet-unlock-password-file/s/^..//' $HOME/.lit/lit.conf
sed -i '/^; wallet-unlock-allow-create/s/^..//' $HOME/.lit/lit.conf
else
gsed -i '/^; wallet-unlock-password-file/s/^..//' $HOME/.lit/lit.conf
gsed -i '/^; wallet-unlock-allow-create/s/^..//' $HOME/.lit/lit.conf
fi
