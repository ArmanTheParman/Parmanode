...
# For Linux...
search_string="$2"
input_file="$(readlink -f "$1")" >$dn 2>&1 # to assign absolute path
echo "delete line $search_string, input file $input_file" | tee -a $dp/sed.log >$dn 2>&1  
sudo sed -i "/${search_string}/d" "$input_file" | tee -a $dp/sed.log >$dn 2>&1
