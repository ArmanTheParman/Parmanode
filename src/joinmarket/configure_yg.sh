...
case $enter_cont in
"")
sudo gsed -i -E 's/minsize =.*$/minsize = 1000000/' $jmcfg 
break
;;
*)
[[ $enter_cont -gt 0 ]] || { invalid && continue ; }
sudo gsed -i -E "s/minsize =.*$/minsize = $enter_cont/" $jmcfg
break
;;
esac
done
...
case $enter_cont in
"")
sudo gsed -i -E 's/size_factor =.*$/size_factor = 0.1/' $jmcfg
break
;;
*)
[[ $(echo "$enter_cont >= 0" | bc -l) == 1 && $(echo "$enter_cont <= 1" | bc -l) == 1 ]] || { invalid && continue ; }
sudo gsed -i -E "s/size_factor =.*$/size_factor = $enter_cont/" $jmcfg
break
;;
esac
done
...
while true ; do
    ...
    "r" "relative" "abs" "absolute" \
        && { sudo gsed -i -E "s/ordertype =.*$/ordertype = reloffer/" $jmcfg ; ordertype=r ; } 
    }   || { sudo gsed -i -E "s/ordertype =.*$/ordertype = absoffer/" $jmcfg ; ordertype=a ; }
    ...
    if [[ $ordertype == r ]] ; then
        ...
        if  [[ $(echo "$enter_cont > 0" | bc -l) == 1 && $(echo "$enter_cont < 1" | bc -l) ]] ; then
            sudo gsed -i -E "s/cjfee_r =.*$/cjfee_r = $enter_cont/" $jmcfg
            break
        else
            invalid
            continue
        ...
        sudo gsed -i -E "s/cjfee_a =.*$/cjfee_a = $enter_cont/" $jmcfg
        break
    fi
...
case $enter_cont in
"")
sudo gsed -i -E 's/cjfee_factor =.*$/cjfee_factor = 0.1/' $jmcfg
break
;;
*)
[[ $(echo "$enter_cont >= 0" | bc -l) == 1 && $(echo "$enter_cont <= 1" | bc -l) == 1 ]] || { invalid ; continue ; }
sudo gsed -i -E "s/cjfee_factor =.*$/cjfee_factor = $enter_cont/" $jmcfg
break
;;
esac
done
