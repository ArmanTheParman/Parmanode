...
if [[ "$positionnewline" == "after" ]] ; then
    ...
    echo $newline | tee -a $tmp/temp1.txt >$dn
    ...
    #condition where there's nothing to add because nothing comes after the searchstring
        cat $tmp/temp1.txt | sudo tee $inputfile >$dn
        #file done
    else
...
if [[ "$positionnewline" == "before" ]] ; then
    ...
    if [[ $count == 1 ]] ; then
        ...
        echo "$newline" | tee -a $tmp/temp2.txt >$dn
        sudo grep -A 100000 "${searchstring}" ${inputfile} | sudo tee -a $tmp/temp2.txt >$dn
        cat $tmp/temp2.txt | sudo tee $inputfile >$dn
        #file done
    fi
fi
...
if [[ "$positionnewline" == "swap" ]] ; then
    ...
    if [ $count == 1 ] ; then # means searchstring is on line one of input file.
        ...
        sudo tail -n $(($count_inputfile -1)) $inputfile | tee $tmp/temp2.txt >$dn
        #temp2 holds inputfile without first line now
        echo "$newline" | tee $tmp/temp3.txt >$dn
        cat $tmp/temp3.txt $tmp/temp2.txt | sudo tee $inputfile >$dn
        #file done
    else
...
if [[ "$positionnewline" == "delete" ]] ; then
    ...
    sudo grep -v "$searchstring" $inputfile | tee $tmp/temp1.txt >$dn
    cat $tmp/temp1.txt | sudo tee $inputfile >$dn
fi

cd $tmp 
rm temp1.txt temp2.txt temp3.txt temp4.txt >$dn 2>&1
cd $pn
}